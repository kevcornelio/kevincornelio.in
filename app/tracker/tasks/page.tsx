...
const blank: Omit<Task, 'id' | 'user_id' | 'created_at'> = {
  title: '',
  description: '',
  status: 'todo',
  priority: 'medium',
  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <button
          onClick={openNew}
          className="rounded-lg bg-primary-500 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-primary-600 dark:bg-primary-400 dark:text-gray-900 dark:hover:bg-primary-500"
        >
          + New task
        </button>
      </div>
        ...
        <ul className="space-y-2">
          {filtered.map((task) => {
            ...
            return (
              <li
                key={task.id}
                className="flex items-start gap-3 rounded-xl border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900"
              >
                <button
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {task.description && (
                    <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400 line-clamp-1">{task.description}</p>
                  )}
                  <div className="mt-1.5 flex flex-wrap gap-1.5">
                    ...
                    {task.due_date && (
                      <span className={`rounded-full px-2 py-0.5 text-xs ${isOverdue ? 'bg-red-100 text-red-600 dark:bg-red-900/40 dark:text-red-400' : 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400'}`}>
