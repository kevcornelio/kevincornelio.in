    ...
    <div>
      ...
      {!child && !showChildForm && (
        <div className="flex h-40 flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-gray-200 text-gray-400 dark:border-gray-700">
          ...
          <button onClick={() => setShowChildForm(true)} className="rounded-lg bg-primary-500 px-4 py-2 text-sm font-semibold text-white dark:bg-primary-400 dark:text-gray-900">Add child</button>
        </div>
      )}
      ...
      {child && (
        <>
            ...
            <div className="ml-auto flex items-center gap-4">
              ...
              <div className="hidden w-32 sm:block">
                <div className="h-2 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800">
                  <div className="h-2 rounded-full bg-primary-500 dark:bg-primary-400 transition-all" style={{ width: `${Math.round((givenCount / totalCount) * 100)}%` }} />
                </div>
                <p className="mt-0.5 text-right text-xs text-gray-400">{Math.round((givenCount / totalCount) * 100)}%</p>
              </div>
              <button onClick={() => handleDeleteChild(child.id)} className="text-xs text-gray-400 hover:text-red-500 transition" title="Remove child">Remove</button>
            </div>
          </div>

          {overdue.length > 0 && (
            <div className="mb-5 rounded-xl border border-red-200 bg-red-50 p-4 dark:border-red-800/50 dark:bg-red-900/20">
              <p className="mb-2 text-sm font-semibold text-red-700 dark:text-red-400">⚠ Action needed ({overdue.length} vaccine{overdue.length > 1 ? 's' : ''})</p>
          ...
          {recordEntry && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
              <div className="w-full max-w-md rounded-xl bg-white p-6 dark:bg-gray-900">
                ...
                <p className="mb-4 text-sm text-gray-500 dark:text-gray-400">{recordEntry.vaccine} — Dose {recordEntry.dose} of {recordEntry.series}</p>
                <div className="space-y-3">
                  <div>
                    <label className="mb-1 block text-xs text-gray-500 dark:text-gray-400">Date given *</label>
                    <input type="date" className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-800 dark:text-white focus:outline-none" value={recordForm.date_given} onChange={(e) => setRecordForm({ ...recordForm, date_given: e.target.value })} />
                  </div>
                  <div>
                    <label className="mb-1 block text-xs text-gray-500 dark:text-gray-400">Provider / clinic</label>
                    <input className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-800 dark:text-white focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500" placeholder="e.g. Dr. Smith, City Clinic" value={recordForm.provider} onChange={(e) => setRecordForm({ ...recordForm, provider: e.target.value })} />
                  </div>
                  <div>
                    <label className="mb-1 block text-xs text-gray-500 dark:text-gray-400">Lot number</label>
                    <input className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-800 dark:text-white focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500" placeholder="From vaccine vial label" value={recordForm.lot_number} onChange={(e) => setRecordForm({ ...recordForm, lot_number: e.target.value })} />
                  </div>
                  <div>
                    <label className="mb-1 block text-xs text-gray-500 dark:text-gray-400">Notes</label>
                    <textarea className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-800 dark:text-white focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500" placeholder="Reactions, observations…" rows={2} value={recordForm.notes} onChange={(e) => setRecordForm({ ...recordForm, notes: e.target.value })} />
                  </div>
                </div>
                <div className="mt-4 flex justify-end gap-2">
                  ...
                  <button onClick={handleSaveRecord} disabled={savingRecord || !recordForm.date_given} className="rounded-lg bg-primary-500 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50 dark:bg-primary-400 dark:text-gray-900">{savingRecord ? 'Saving…' : 'Save record'}</button>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="mb-3 flex items-center justify-between">
            ...
            <button onClick={() => setShowAll(!showAll)} className="text-xs text-primary-500 hover:underline dark:text-primary-400">{showAll ? 'Show relevant only' : 'Show full schedule'}</button>
          </div>

          <div className="space-y-4">
            {relevantGroups.map((group) => {
              if (group.vaccines.length === 0) return null
              const groupStatuses = group.vaccines.map((e) => getScheduleStatus(e, ageMonths, recordMap.get(e.key)))
              ...
              return (
                <div key={group.label}>
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <h4 className="text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">{group.label}</h4>
                  ...
                  <div className="ml-4 space-y-1.5">
                    {group.vaccines.map((entry) => {
                      ...
                      return (
                        <div key={entry.key} className="flex items-center gap-3 rounded-lg border border-gray-100 bg-white px-3 py-2.5 dark:border-gray-700/50 dark:bg-gray-900">
                          <span className={`h-2 w-2 shrink-0 rounded-full ${cfg.dot}`} />
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="text-sm text-gray-800 dark:text-gray-200">
                                {entry.vaccine}
                                {entry.series > 1 && <span className="ml-1 text-xs text-gray-400">(dose {entry.dose}/{entry.series})</span>}
                              </span>
                            ...
                            {entry.notes && !record && <p className="mt-0.5 text-xs text-gray-400 italic">{entry.notes}</p>}
                          </div>
                          {status === 'given' ? (
                            <button onClick={() => handleDeleteRecord(entry.key)} className="shrink-0 text-xs text-gray-300 hover:text-red-400 transition dark:text-gray-600" title="Undo record">✕</button>
                          ) : (
                            <button
